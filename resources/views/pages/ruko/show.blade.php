...
@section('title', $ruko->name . ' - CitraGarden')
...
@section('content')
                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('ruko.index') }}" class="text-gray-500 hover:text-gray-700">Daftar Ruko</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                            <span class="text-gray-900">{{ $ruko->name }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        @else
                            <div class="h-96 bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center">
                                <i class="fas fa-store text-8xl text-gray-500"></i>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $ruko->name }}</h1>
                        ...
                        <div class="text-right">
                            <p class="text-3xl font-bold text-indigo-600">{{ $ruko->formatted_price }}</p>
                            <p class="text-gray-500">Bisa Nego</p>
                            <span class="inline-block mt-2 px-3 py-1 text-sm font-medium rounded-full 
                                {{ $ruko->status == 'available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $ruko->status == 'available' ? 'Tersedia' : 'Sudah Dipesan' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div class="text-center p-4 bg-gray-50 rounded-lg">
                            <i class="fas fa-building text-2xl text-indigo-500 mb-2"></i>
                            <p class="font-semibold text-gray-900">{{ $ruko->floors }}</p>
                            <p class="text-sm text-gray-500">Lantai</p>
                        </div>
                        <div class="text-center p-4 bg-gray-50 rounded-lg">
                            <i class="fas fa-ruler-combined text-2xl text-indigo-500 mb-2"></i>
                            <p class="font-semibold text-gray-900">{{ $ruko->area }} m²</p>
                            <p class="text-sm text-gray-500">Luas Area</p>
                        </div>
                        <div class="text-center p-4 bg-gray-50 rounded-lg">
                            <i class="fas fa-parking text-2xl text-indigo-500 mb-2"></i>
                            <p class="font-semibold text-gray-900">{{ $ruko->parking }}</p>
                            <p class="text-sm text-gray-500">Parkir</p>
                        </div>
                        <div class="text-center p-4 bg-gray-50 rounded-lg">
                            <i class="fas fa-store text-2xl text-indigo-500 mb-2"></i>
                            <p class="font-semibold text-gray-900">{{ $ruko->subtype }}</p>
                            <p class="text-sm text-gray-500">Tipe Ruko</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-gray-700 leading-relaxed">{{ $ruko->description }}</p>
                    </div>

                    <!-- Amenities -->
                    @if($ruko->amenities && count($ruko->amenities) > 0)
                    <div class="mb-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-3">Fasilitas</h3>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($ruko->amenities as $amenity)
                    ...
                    <!-- Features -->
                    @if($ruko->features && count($ruko->features) > 0)
                    <div class="mb-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-3">Fitur Unggulan</h3>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($ruko->features as $feature)
                            <div class="flex items-center text-gray-700">
                ...
                <!-- Location -->
                @if($ruko->latitude && $ruko->longitude)
                {{-- <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 mb-6 sticky top-24">
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">Booking Ruko Ini</h3>
                    
                    <div class="mb-6">
                        <p class="text-3xl font-bold text-indigo-600 mb-2">{{ $ruko->formatted_price }}</p>
                        <p class="text-gray-500">per bulan</p>
                    </div>

                    @if($ruko->status == 'available')
                        @auth
                            <a href="{{ route('bookings.create', $ruko->id) }}" 
                    ...
                    <div class="flex space-x-2">
                        <a href="https://wa.me/62895349132500?text=Halo, saya tertarik dengan {{ $ruko->name }}" 
                           target="_blank"
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <p class="font-medium text-gray-900">{{ $ruko->agent_name ?? 'Agen CitraGarden' }}</p>
    ...
    <div x-data="{ isOpen: false, currentImage: 0 }" 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <div class="relative">
                @if($ruko->images && count($ruko->images) > 0)
                    <template x-for="(image, index) in {{ json_encode($ruko->images) }}" :key="index">
                ...
                @if($ruko->images && count($ruko->images) > 1)
                    <button @click="currentImage = (currentImage - 1 + {{ count($ruko->images) }}) % {{ count($ruko->images) }}"
                    ...
                    <button @click="currentImage = (currentImage + 1) % {{ count($ruko->images) }}"
